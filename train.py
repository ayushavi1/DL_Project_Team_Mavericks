        ...
        with h5py.File(train_set.cache, mode='w') as h5: 
            ...
            with torch.no_grad():
                ...
                if opt.matcher is not None:
                    ...
                    if opt.matcher == 'mavNet':
                        if 'nordland' in opt.dataset.lower() or 'tmr' in opt.dataset.lower():
                            dMat_cont = mavNet.mavNet.computeDisMat_torch(dbFeat[:offset,outSeqL//2], dbFeat[offset:,outSeqL//2])
                            if opt.neg_trip_method == 'centerOnly':
                                h5DisMat[...] = dMat_cont.detach().cpu().numpy()
                            elif opt.neg_trip_method == 'meanOfPairs':
                                h5DisMat[...] = mavNet.aggregateMatchScores(dMat_cont,outSeqL,device,dMatProcOneShot=False)[0] * (1.0/outSeqL)
                        else:
                            if opt.neg_trip_method == 'centerOnly':
                                h5DisMat[...] = mavNet.aggregateMatchScores(None,1,device, dbDesc=dbFeat[:offset,outSeqL//2:outSeqL//2+1], qDesc=dbFeat[offset:,outSeqL//2:outSeqL//2+1])[0]
                            elif opt.neg_trip_method == 'meanOfPairs':
                                h5DisMat[...] = mavNet.aggregateMatchScores(None,outSeqL,device, dbDesc=dbFeat[:offset], qDesc=dbFeat[offset:])[0] * (1.0/outSeqL)
                    else:
                        raise("TODO")
