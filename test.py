...
def test(opt,model_eff_net, model, encoder_dim, device, eval_set, writer, epoch=0, extract_noEval=False):
    ...
    if not opt.useEffnet :
        with torch.no_grad():
            ...
            for iteration, (input, indices) in tqdm(enumerate(test_data_loader, 1),total=len(test_data_loader)-1, leave=False):
                ...
                effnet_global_single_descs = model_eff_net(image_encoding).squeeze()
                dbFeat_single[indices] = global_single_descs
                effnet_dbFeat_single[indices] = effnet_global_single_descs
